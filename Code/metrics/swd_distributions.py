...
class API:
    ...
    def feed(self, mode, images, num_images_per_group, num_groups, snapshot_kimg, result_subdir):
        groups_lap = []
        for i in range(num_groups * 2):
            minibatch = images[i * num_images_per_group : (i + 1) * num_images_per_group]
            descriptors = [[] for res in self.resolutions]
        ...
        kk = np.tril(np.ones((num_groups * 2,num_groups * 2)), -1)  
        ...
        x_real_co = coos[:num_groups, 0]
        y_real_co = coos[:num_groups, 1]
        x_fake_co = coos[num_groups:, 0]
        y_fake_co = coos[num_groups:, 1]  
        ...
        axes[0].plot(x_real_co, y_real_co, 'ro', label = 'Training')
        axes[0].plot(x_fake_co, y_fake_co, 'b+', label = 'Generated')
        axes[0].legend(loc='upper left')
        
        # Evaluate a gaussian kde on a regular grid of nbins x nbins over data extents
        nbins = 40
        k_real = kde.gaussian_kde((coos.T[:, :num_groups]))
        ...
        real_contr = axes[1].contour(xi_real, yi_real, zi_real.reshape(xi_real.shape), 4, colors='r', label = 'Training') 

        k_fake = kde.gaussian_kde((coos.T[:, num_groups:]))
        xi_fake, yi_fake = np.mgrid[plot_lim_min:plot_lim_max:nbins*1j, plot_lim_min:plot_lim_max:nbins*1j]
        zi_fake = k_fake(np.vstack([xi_fake.flatten(), yi_fake.flatten()]))
        fake_contr = axes[1].contour(xi_fake, yi_fake, zi_fake.reshape(xi_fake.shape), 4, colors='blue', label = 'Generated', linestyles= 'dashed') 
        axes[1].set_title('Densityplot')
        real_contr.collections[0].set_label('Training')
        fake_contr.collections[0].set_label('Generated')
        axes[1].legend(loc='upper left') 
        
        plt.savefig(result_subdir + '/snapshot swd distribution %-6d.jpg' % snapshot_kimg, dpi=200)
