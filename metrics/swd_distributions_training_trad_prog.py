...
class API:
    ...
    def feed(self, mode, images, num_images_per_group, num_groups, result_subdir):
        groups_lap = []
        ...
        kk = np.tril(np.ones((num_groups * 3,num_groups * 3)), -1)  
        ...
        x_fake_trad_co = coos[num_groups:num_groups*2, 0]
        y_fake_trad_co = coos[num_groups:num_groups*2, 1]  
        x_fake_prog_co = coos[num_groups*2:, 0]
        y_fake_prog_co = coos[num_groups*2:, 1] 
        ...
        axes[0].plot(x_fake_trad_co, y_fake_trad_co, 'b+', label = 'Conventional')
        axes[0].plot(x_fake_prog_co, y_fake_prog_co, 'k^', label = 'Progressive')
        ...
        k_fake_trad = kde.gaussian_kde((coos.T[:, num_groups:num_groups*2]))
        ...
        fake_contr_trad.collections[0].set_label('Conventional')
        fake_contr_prog.collections[0].set_label('Progressive')
        axes[1].legend(loc='upper right') 
        
        plt.savefig(result_subdir + '/final swd distribution_training_conventional_progressive_1.jpg' , dpi=200)
